...
exports.status = function() {
	exec('git status', function(error, stdout, stderr) {
		...
		// Branch.
		console.log(chalk.black.dim('branch  ') + chalk.gray.dim(' | ') + success(status[0].split(' ')[2]));

		// Position in branch.
		var position;
		switch (status[1].split(' ')[3]) {
			case 'behind':
				position = chalk.black.dim.bold('position') + chalk.gray.dim(' | ') + notice.bold('behind');
				break;

			case 'up-to-date':
				position = chalk.black.dim('position') + chalk.gray.dim(' | ') + success('up-to-date');
				break;

			case 'ahead':
				position = chalk.black.dim.bold('position') + chalk.gray.dim(' | ') + notice.bold('ahead');
				break;

			default:
				position = chalk.black.dim.bold('position') + chalk.gray.dim(' | ') + whoops('diverged');
		}
