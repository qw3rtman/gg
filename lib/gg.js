...
exports.status = function() {
	exec('git status', function(error, stdout, stderr) {
		...
		switch (status[1].split(' ')[3]) {
			case 'behind':
				position = 'position' + chalk.gray.dim(' | ') + notice(status[1].split(' ')[7] + ' commits behind')  + notice(' (fetch or pull)');
				break;
			...
			case 'ahead':
				position = 'position' + chalk.gray.dim(' | ') + notice(status[1].split(' ')[7] + ' commits ahead') + notice(' (push commits)');
				break;
